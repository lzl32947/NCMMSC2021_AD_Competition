...
from util.wav_process.audio_tools import *
import numpy as np
import pylab
import os
...
if __name__ == '__main__':
    path = r'./../../dataset/merge/AD/AD_F_030807.wav'
    fs, s = read_wav(path)
    ...
    # dir = ['AD', 'MCI', 'HC']
    # for i in range(3):
    #     for temp in os.listdir(path + "/" + dir[i]):
    #         test_file = path + "/" + dir[i] + "/" + temp
    ...
    #
    #         processed_wav = s1 * s
    #         processed_wav = np.delete(processed_wav, np.where(processed_wav == 0.))
    #
    #         write_wav("./../../dataset" + "/" + 'merge_vad' + "/" + dir[i] + "/" + temp, processed_wav)
